...
var fizzBuzz = function(n) {
	...
	return fizzBuzz(n - 1).concat(convert(n));
};
...
var countOccurrence = function(array, value) {
	...
	return array[0] === value || isNaN(array[0]) && isNaN(value)
		? 1 + countOccurrence(array.slice(1), value)
		: 0 + countOccurrence(array.slice(1), value);
};
...
var fibonacci = function(n) {
	...
	// recursive case: add previous two numbers to get next number
	var a = fibonacci(n-1);
	return a.concat(a[a.length-2] + a[a.length-1]);
};
...
var augmentElements = function(array, aug) {
	...
	array.forEach(a => {
		result.push(augmentElements(a, aug));
	});
...
var alternateSign = function(array) {
	...
	// array has two or more elements left
	if ( array.length >= 2 ) {
		// first should be pos, second should be neg
		if ( array[0] < 0 ) {
			array[0] *= -1;
		}
		if ( array[1] > 0 ) {
	...
	if ( array.length === 1 ) {
		// should always be positive since it must be odd-indexed
		if ( array[0] < 0 ) {
			array[0] *= -1;
			return array;
		}
...
var numToText = function(str) {
	...
	if ( !isNaN(parseInt(str.charAt(0))) ) {
		return dict[str.charAt(0)] + numToText(str.slice(1));
	} else {
